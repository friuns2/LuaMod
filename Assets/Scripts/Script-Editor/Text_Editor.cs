...
";
    ...
    public class Activate_Scheme_Info
    {
        ...
        public enum Activate_Enum
        {
            OnClick,
            OnMouseOver,
            Never
        };
        public enum DeActivate_Enum
        {
            OnClickOutside,
            OnMouseOut,
            Never
    ...
    {
        public changed_rows_info(Text r, string txt)
        {
            associated_row = r;
            text = txt;
        }
    ...
    class variable_info
    {
        public variable_info(Text r, classes_dict d)
        {
            row = r;
            classes_dict = d;
        }
        public variable_info(Text r, classes_dict d, int[] c)
        {
            row = r;
            classes_dict = d;
            context = c;
    ...
    void Awake()
    {
        ...
        keyinfo.Add(KeyCode.Minus, '-');
        keyinfo.Add(KeyCode.Equals, '=');
        keyinfo.Add(KeyCode.Backslash, '\\');
        keyinfo.Add(KeyCode.LeftBracket, '[');
        keyinfo.Add(KeyCode.RightBracket, ']');
        keyinfo.Add(KeyCode.Semicolon, ';');
        keyinfo.Add(KeyCode.Quote, '\'');
        keyinfo.Add(KeyCode.Comma, ',');
        keyinfo.Add(KeyCode.Period, '.');
        keyinfo.Add(KeyCode.Slash, '/');
        keyinfo.Add(KeyCode.Space, ' ');
        keyinfo.Add(KeyCode.Tab, '\t');
        keyinfo.Add(KeyCode.KeypadDivide, '/');
        keyinfo.Add(KeyCode.KeypadMultiply, '*');
        keyinfo.Add(KeyCode.KeypadMinus, '-');
        keyinfo.Add(KeyCode.KeypadPlus, '+');
        keyinfo.Add(KeyCode.KeypadPeriod, '.');
        ...
        keyUpper.Add(KeyCode.Minus, '_');
        keyUpper.Add(KeyCode.Equals, '+');
        keyUpper.Add(KeyCode.Backslash, '|');
        keyUpper.Add(KeyCode.LeftBracket, '{');
        keyUpper.Add(KeyCode.RightBracket, '}');
        keyUpper.Add(KeyCode.Semicolon, ':');
        keyUpper.Add(KeyCode.Quote, '"');
        keyUpper.Add(KeyCode.Comma, '<');
        keyUpper.Add(KeyCode.Period, '>');
        keyUpper.Add(KeyCode.Slash, '?');
        ...
        tex2.SetPixel(0, 0, new Color32(128, 128, 255, 200));
        tex2.Apply();
        ...
        builtin_types.Add("bool", "System.Boolean");
        builtin_types.Add("byte", "System.Byte");
        builtin_types.Add("sbyte", "System.SByte");
        builtin_types.Add("char", "System.SByte");
        builtin_types.Add("decimal", "System.Decimal");
        builtin_types.Add("double ", "System.Double");
        builtin_types.Add("float", "System.Single");
        builtin_types.Add("int", "System.Int32");
        builtin_types.Add("uint", "System.UInt32");
        builtin_types.Add("long", "System.Int64");
        builtin_types.Add("ulong", "System.UInt64");
        builtin_types.Add("object", "System.Object");
        builtin_types.Add("short", "System.Int16");
        builtin_types.Add("ushort", "System.UInt16");
        builtin_types.Add("string", "System.String");
        var keys = builtin_types.Keys.ToArray();
        ...
        if (ScrollH != null)
        {
            tece = ScrollH.gameObject.AddComponent<Text_Editor_Child_Element>();
            tece.te = this;
        }
        if (ScrollV != null)
        {
            tece = ScrollV.gameObject.AddComponent<Text_Editor_Child_Element>();
            tece.te = this;
        }
        if (Intellisense_Settings.suggestion_rt != null)
        {
            tece = Intellisense_Settings.suggestion_rt.gameObject.AddComponent<Text_Editor_Child_Element>();
            tece.te = this;
        }
        if (Intellisense_Settings.suggestion_item != null)
        {
            var tesi = Intellisense_Settings.suggestion_item.gameObject.AddComponent<Text_Editor_Suggestion_Item>();
            tesi.te = this;
        }

        check_var_thread.DoWork += Intellisense_var_check_bg;
        check_var_thread.RunWorkerAsync();

        if (Activate_Settings.Activate != Activate_Scheme_Info.Activate_Enum.OnMouseOver)
        {
            if (Activate_Settings.IsActive) Activate();
            else DeActivate();
        }
        else
        {
    ...
    void Update()
    {
        ...
        if (Engine.check_key(Engine.Key.Set_Breakpoint))
        {
            SetBreakpoint();
        }
        if (Engine.check_key(Engine.Key.Intellisense_Enable))
        {
        ...
        if (Engine.check_key(Engine.Key.Debug_Force_Update))
        {
            Intellisense_var_check_bg(null, null);
        }
        ...
        {
            if ((int) repeat_key < 1000)
            {
                if (!Input.GetKey(repeat_key)) repeat_start_timer = -1f;
            }
            else if ((int) repeat_key == 10001)
            {
                if (!Engine.check_key(Engine.Key.Intellisense_Up, false)) repeat_start_timer = -1f;
            }
            else if ((int) repeat_key == 10002)
            {
                if (!Engine.check_key(Engine.Key.Intellisense_Down, false))
                {
                    repeat_start_timer = -1f;
                }
            }
        }
        ...
        if (Intellisense_Settings.suggestion_rt != null && Intellisense_Settings.suggestion_rt.gameObject.activeSelf)
        {
            ...
            if (Engine.check_key(Engine.Key.Intellisense_Submit))
            {
                Intellisense_Navigation(nav_enum.Submit);
                return;
            }
            if (repeat == (KeyCode) 10001 || Engine.check_key(Engine.Key.Intellisense_Up))
            {
                Intellisense_Navigation(nav_enum.Up);
                suggestion_mouse_enabled = false;
            }
            if (repeat == (KeyCode) 10002 || Engine.check_key(Engine.Key.Intellisense_Down))
            {
                Intellisense_Navigation(nav_enum.Down);
                suggestion_mouse_enabled = false;
            }
        }
            ...
            foreach (var ki in keyinfo)
            {
                if (Input.GetKeyDown(ki.Key) || repeat == ki.Key)
                {
                    ...
                    if (Auto_Close_Pairs_Dict.ContainsValue(type) && Selection_Get_Text() == "")
                    {
                        ...
                        if (t.Length > current_chr && t[current_chr] == type[0])
                        {
                            current_chr++;
                            Cursor_Update_Position();
                            return;
                    ...
                    {
                        type += Auto_Close_Pairs_Dict[type];
                        pair_autoclosed = true;
                    }
        ...
        if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter) || repeat == KeyCode.Return || repeat == KeyCode.KeypadEnter)
        {
            if (!alt_Pressed)
            {
                ...
                Close_Intellisense();
                if (Input.GetKeyDown(KeyCode.Return)) Character_Repeat_Handler("SET", KeyCode.Return);
                else Character_Repeat_Handler("SET", KeyCode.KeypadEnter);
            }
            else
            {
                Intellisense_Navigation(nav_enum.Submit);
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow) || repeat == KeyCode.LeftArrow)
        {
            Navigate(nav_enum.Left);
        }
        if (Input.GetKeyDown(KeyCode.RightArrow) || repeat == KeyCode.RightArrow)
        {
            Navigate(nav_enum.Right);
        }
        if (Input.GetKeyDown(KeyCode.UpArrow) || repeat == KeyCode.UpArrow)
        {
            if (!alt_Pressed) Navigate(nav_enum.Up);
        }
        if (Input.GetKeyDown(KeyCode.DownArrow) || repeat == KeyCode.DownArrow)
        {
            if (!alt_Pressed) Navigate(nav_enum.Down);
        }
        if (Input.GetKeyDown(KeyCode.PageUp) || repeat == KeyCode.PageUp)
        {
            Navigate(nav_enum.PgUp);
        }
        if (Input.GetKeyDown(KeyCode.PageDown) || repeat == KeyCode.PageDown)
        {
            Navigate(nav_enum.PgDn);
        }
        if (Input.GetKeyDown(KeyCode.Home))
        {
            Navigate(nav_enum.Home);
        }
        if (Input.GetKeyDown(KeyCode.End))
        {
            Navigate(nav_enum.End);
        }
        if (Input.GetKeyDown(KeyCode.Home) && ctrl_Pressed)
        {
            Navigate(nav_enum.Ctrl_Home);
        }
        if (Input.GetKeyDown(KeyCode.End) && ctrl_Pressed)
        {
            Navigate(nav_enum.Ctrl_End);
        }
        if (Input.mouseScrollDelta.y != 0f)
        {
            ...
            {
                Navigate(nav_enum.Mouse_Scroll);
            }
            else
            {
                Navigate(nav_enum.Mouse_ScrollH);
            }
        }
        ...
        if (Input.GetKeyDown(KeyCode.Tab) && (selection_start[0] != selection_end[0] || shift_pressed))
        {
            ...
            else
            {
                ...
                for (int i = s[0]; i <= e[0]; i++)
                {
                    ...
                    if (i == current_row)
                    {
                        if (current_chr >= count) current_chr -= count;
                        else current_chr = 0;
                    }
                }
    ...
    void Navigate(nav_enum nav)
    {
        ...
        {
            current_chr--;
            Character_Repeat_Handler("SET", KeyCode.LeftArrow);
        ...
        {
            current_chr++;
            Character_Repeat_Handler("SET", KeyCode.RightArrow);
        ...
        {
            current_row -= Mathf.FloorToInt(txt_cnt_rt.rect.height / lineHeight);
            Character_Repeat_Handler("SET", KeyCode.PageUp);
        }
        else if (nav == nav_enum.PgDn)
        {
            current_row += Mathf.FloorToInt(txt_cnt_rt.rect.height / lineHeight);
            Character_Repeat_Handler("SET", KeyCode.PageDown);
        }
        ...
        Cursor_Update_Position(dont_update_previous);
        Cursor_Blink(true);
    ...
    void Cursor_Blink(bool reset = false)
    {
        if (!Activate_Settings.IsActive)
        {
            if (Activate_Settings.Hide_Cursor_When_Inactive)
            {
    ...
    void Cursor_Update_Position(bool dont_update_previous = false)
    {
        ...
        if (Intellisense_Settings.suggestion_rt != null)
        {
            ...
            {
                new_pos.x = limit_X;
                new_pos.y -= lineHeight;
            }
            if (new_pos.y < limit_Y)
            {
                new_pos.y = pos.y + Intellisense_Settings.suggestion_rt.rect.height + 10f;
            }
    ...
    void Selection_Draw(int[] start, int[] end)
    {
        ...
        {
            if (!used_selectors.Contains(kv.Key))
            {
    ...
    void Undo_Restore()
    {
        ...
        if (ls.Count > 0)
        {
            current_row = ls[0].cursor_pos.x;
            current_chr = ls[0].cursor_pos.y;
        }
        ...
        for (int i = ls.Count - 1; i >= 0; i--)
        {
            ...
            if (cur.operation == undo_op.modify)
            {
                ...
                text_char_w[cur.line_ind].Clear();
                text_rows_w[cur.line_ind] = 0;
                current_row = cur.line_ind;
                current_chr = 0;
    ...
    void Syntax_Highlight(int row)
    {
        ...
        for (int i = 0; i < txt.Length; i++)
        {
            ...
            if (c == '"' && group_cur != 2)
            {
                ...
                txt_new += "<color=#" + ColorUtility.ToHtmlStringRGBA(Color_Scheme.t_quote) + ">";
                is_inside_color_tag = true;
                group_cur = 2;
            }
            ...
            if (group_cur != group_prev && group_cur != 2)
            {
                ...
                if (group_cur == 0)
                {
                    txt_new += "<color=#" + ColorUtility.ToHtmlStringRGBA(Color_Scheme.t_std) + ">";
                    is_inside_color_tag = true;
                }
                if (group_cur == 1)
                {
                    txt_new += "<color=#" + ColorUtility.ToHtmlStringRGBA(Color_Scheme.t_sym) + ">";
                    is_inside_color_tag = true;
                }
            }
            ...
            foreach (Assembly asm in assemblies)
            {
                ...
                foreach (var nmsp in namespaces_arr)
                {
                    ...
                    foreach (Type type in types)
                    {
                        ...
                        {
                            if (!current_level_dict.dict.ContainsKey(n))
                            {
                                current_level_dict.dict.Add(n, new classes_dict());
                            }
                        ...
                        {
                            ttt = "Class";
                            current_level_dict.type = classes_dict.Type_Enum.T_class;
                        }
                        if (type.IsEnum)
                        {
                            ttt = "Enum";
                            current_level_dict.type = classes_dict.Type_Enum.T_enum;
                        }
                        if (type.IsInterface)
                        {
                            ttt = "Interface";
                            current_level_dict.type = classes_dict.Type_Enum.T_Interface;
                        }
                        if (type.IsValueType && !type.IsEnum)
                        {
                            ttt = "Struct";
                            current_level_dict.type = classes_dict.Type_Enum.T_struct;
                        }
                        if (type.BaseType == typeof(MulticastDelegate))
                        {
                            ttt = "Delegate";
                            current_level_dict.type = classes_dict.Type_Enum.T_Delegate;
                        }
                        ...
                        if (type.IsEnum)
                        {
                            foreach (string s in type.GetEnumNames())
                            {
                                ...
                                if (!current_level_dict.dict.ContainsKey(s))
                                {
                                    ...
                                    ncd.type = classes_dict.Type_Enum.Enum_Value;
                        ...
                        foreach (var mi in methodInfos)
                        {
                            ...
                            {
                                var ncd = new classes_dict();
                                ncd.type = classes_dict.Type_Enum.M_Method;
                                ncd.is_static = mi.IsStatic;
                                ncd.return_type = mi.ReturnType.Name;
                                ncd.return_type_full = mi.ReturnType.FullName;
                                ...
                                {
                                    // && (t_name.ToLower().EndsWith("list") || t_name.ToLower().EndsWith("dictionary"))) {
                                ...
                                if (mi.IsDefined(typeof(System.Runtime.CompilerServices.ExtensionAttribute), false))
                                {
                                    ...
                                    {
                                        if (!current_level_dict_for_ext.dict.ContainsKey(n))
                                        {
                                            current_level_dict_for_ext.dict.Add(n, new classes_dict());
                                        }
                        ...
                        foreach (var pi in propertyInfos)
                        {
                            ...
                            if (!current_level_dict.dict.ContainsKey(pi.Name))
                            {
                                ...
                                ncd.return_type = pi.PropertyType.Name;
                                ncd.return_type_full = pi.PropertyType.FullName;
                        ...
                        {
                            var ncd = new classes_dict();
                            ncd.type = classes_dict.Type_Enum.P_Indexer;
                            ncd.is_static = false;
                            ncd.return_type = "T";
                            ncd.return_type_full = "T";
                        ...
                        foreach (var fi in fieldInfos)
                        {
                            ...
                            ttt = "NotSet";
                            var ncd = new classes_dict();
                            ncd.type = classes_dict.Type_Enum.F_Field;
                            ...
                            {
                                ttt = "ReadOnly";
                                ncd.type = classes_dict.Type_Enum.F_READONLY;
                            }
                            if (fi.IsLiteral && !fi.IsInitOnly)
                            {
                                ttt = "Const";
                                ncd.type = classes_dict.Type_Enum.F_CONST;
                            }
                        ...
                        foreach (var ei in eventInfos)
                        {
                            ...
                            {
                                var ncd = new classes_dict();
                                ncd.type = classes_dict.Type_Enum.E_Event;
                ...
                if (WRITE_TIMELOG)
                {
                    sw_T.WriteLine("Got all types of those namespaces in " + (DateTime.Now - test_time).TotalSeconds + "sec. Types count: " + t_count);
                    sw_T.WriteLine("");
                }
    ...
    void Intellisense_Init_Derived_Recour(classes_dict cl)
    {
        foreach (var kv in cl.dict)
        {
            for (int d = 0; d < kv.Value.derived_from.Count; d++)
            {
                ...
                if (!d_mod.Contains("]]"))
                {
                    //TODO: Nested T i.e. - System.Collections.Generic.IEnumerable`1[System.Collections.Generic.KeyValuePair`2[TKey,TValue]]
...
//DONE: var x = new string[][3,456][]{}; x[1][2].--- suggest element instead of array
    ...
    void Intellisense(string text, int chr)
    {
        ...
        for (int i = chr; i >= 0; i--)
        {
            ...
            {
                break;
            } //We found an opening paranthesis without closing contrepart, i.e. GetInput(K...
            if (in_brackets < 0)
            {
                break;
            } //We found an opening bracket without closing contrepart, i.e. GetInput[K...
        ...
        foreach (Match m in paranthesis.Matches(words))
        {
            ...
            paranthesis_length += m.Value.Length;
        }
        ...
        {
            search_list.AddRange(intellisense_usings);
            search_list.Add(intellisense_vars);
            index_of_var_list = search_list.Count() - 1;
        }
        ...
        foreach (classes_dict search_dict in search_list)
        {
            ...
            bool is_variable = (c == index_of_var_list);
            c++;
            ...
            foreach (var kv in search_dict.dict)
            {
                if (last_word == "" || kv.Key.StartsWith(last_word, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        case classes_dict.Type_Enum.N_Namespace:
                            img.sprite = Intellisense_Settings.icon_namespace;
                            break;
                        case classes_dict.Type_Enum.T_Delegate:
                            img.sprite = Intellisense_Settings.icon_delegate;
                            break;
                        case classes_dict.Type_Enum.T_class:
                            img.sprite = Intellisense_Settings.icon_class;
                            break;
                        case classes_dict.Type_Enum.T_Interface:
                            img.sprite = Intellisense_Settings.icon_interface;
                            break;
                        case classes_dict.Type_Enum.T_struct:
                            img.sprite = Intellisense_Settings.icon_struct;
                            break;
                        case classes_dict.Type_Enum.T_enum:
                            img.sprite = Intellisense_Settings.icon_enum;
                            break;
                        case classes_dict.Type_Enum.Enum_Value:
                            img.sprite = Intellisense_Settings.icon_enum_value;
                            break;
                        case classes_dict.Type_Enum.M_Method:
                            img.sprite = Intellisense_Settings.icon_method;
                            break;
                        case classes_dict.Type_Enum.M_ExtMethod:
                            img.sprite = Intellisense_Settings.icon_method_extension;
                            break;
                        case classes_dict.Type_Enum.P_Property:
                            img.sprite = Intellisense_Settings.icon_property;
                            break;
                        case classes_dict.Type_Enum.F_Field:
                            img.sprite = Intellisense_Settings.icon_field;
                            break;
                        case classes_dict.Type_Enum.F_READONLY:
                            img.sprite = Intellisense_Settings.icon_field_readonly;
                            break;
                        case classes_dict.Type_Enum.F_CONST:
                            img.sprite = Intellisense_Settings.icon_field_const;
                            break;
                        default:
                            img.sprite = Intellisense_Settings.icon_not_set;
                            break;
                    }
        ...
        while (true)
        {
            ...
            while (changed_rows.TryDequeue(out rows_Info))
            {
                ...
                foreach (Match match_var_cmn in matches)
                {
                    ...
                    foreach (Match m in brackets.Matches(chain))
                    {
                        chain = chain.Replace(m.Value, "[]");
                    }

                    string chain_out;
                    bool tmp_chain_is_static;
                ...
                foreach (Match match_var_cmn in matches)
                {
                    ...
                    //Replace [] with content by just [] i.e. -> [,] by []
                    foreach (Match m in brackets.Matches(var_chain))
                    {
                        var_chain = var_chain.Replace(m.Value, "[]");
                    }
    ...
    void Intellisense_VariableAddOrUpdate(classes_dict variable_class_dict, string variable_name, Text row, int[] context = null)
    {
        ...
        var ncd = new classes_dict();
        ncd.type = variable_class_dict.type;
        ncd.dict = variable_class_dict.dict;
        ncd.full_name = variable_class_dict.full_name;
        ncd.T2 = variable_class_dict.T2;
    ...
    classes_dict Intellisense_SearchChain(string[] words_arr, out string chain, out bool chain_is_static, bool search_in_vars = true, bool recur = false)
    {
        ...
        for (int i = 0; i < words_arr.Length; i++)
        {
            ...
            while (words_arr[i].EndsWith("[]"))
            {
                is_array++;
                words_arr[i] = words_arr[i].Substring(0, words_arr[i].Length - 2);
            }
            while (words_arr[i].EndsWith("@"))
            {
                is_array_element++;
                words_arr[i] = words_arr[i].Substring(0, words_arr[i].Length - 1);
            }
                ...
                for (int d = 0; d < current_level_dict.derived_from_cl.Count; d++)
                {
                    ...
                    if (cl.dict.ContainsKey(words_arr[i]))
                    {
                        ...
                        foreach (string t in current_level_dict.derived_from[d].Split(new char[] {';'}, StringSplitOptions.RemoveEmptyEntries))
                        {
                            ...
                            if (g.Count() == 1)
                            {
                                current_word_generics.Push(g.First());
                            } //Debug.Log("Push generic def of the base class: " + g.First().Key + " -> " + g.First().Value.full_name); 
                        }
            ...
            for (int g = 0; g < generic_def.Length && g < current_level_dict.T2.Count(); g++)
            {
                if (recur)
                {
                    Debug.Log("Prevent unexpected recursion when trying to get method: " + chain);
                    return null;
                }
            ...
            foreach (var kv in current_level_dict.T2)
            {
                if (kv.Value != null)
                {
                    current_word_generics.Push(kv);
                } //Debug.Log("Push generic def: " + kv.Key); }
            }
            ...
            if (current_level_dict.type == classes_dict.Type_Enum.M_Method || current_level_dict.type == classes_dict.Type_Enum.M_ExtMethod || current_level_dict.type == classes_dict.Type_Enum.P_Property || current_level_dict.type == classes_dict.Type_Enum.F_Field)
            {
                ...
                if (current_level_dict_new == null)
                {
                    ...
                    for (int a = 0; a < make_arr; a++)
                    {
                        current_level_dict = Intellisense_SearchChain_MakeArray(current_level_dict);
                    }
                }
                else
                {
                ...
                for (int a = 0; a < is_array_element; a++)
                {
                    current_level_dict = current_level_dict.T2[0].Value;
                }
    ...
    classes_dict Intellisense_SearchChain_MakeArray(classes_dict class_dict)
    {
        ...
        if (arr_class_dict.T2.Count == 0) arr_class_dict.T2.Add(kv);
        else arr_class_dict.T2[0] = kv;
    ...
    {
        get { return Intellisense_Settings.get_assemblies_from_reflection; }
        set
        {
    ...
    public float line_height { get { return lineHeight; } }
    ...
    public void Color_Scheme_Apply(bool update_syntax_highlight = false)
    {
        ...
        //Cursor
        Texture2D tex = new Texture2D(1, 1);
        tex.SetPixel(0, 0, Color_Scheme.cursor);
        tex.Apply();
        cursor.GetComponent<RawImage>().texture = tex;

        //Selector
        Texture2D tex2 = new Texture2D(1, 1);
        tex2.SetPixel(0, 0, Color_Scheme.selection_color);
        tex2.Apply();
        selector.GetComponent<RawImage>().texture = tex2;
    ...
    public void HighlightText(int start_char, int end_char, Color? color = null)
    {
        ...
        if (start_char < 0)
        {
            ...
            // return;
            HighlightText(new int[] {-1, -1}, new int[] {-1, -1});
            return;
        }
        ...
        for (int r = 0; r < text_char_w.Count; r++)
        {
            ...
            {
                h_end = new int[] {r, end_char - cur_row_start_char};
                break;
            }
    ...
    public void SetBreakpoint()
    {
        ...
        else
        {
            ...
            if (length > 0)
            {
                ...
                pc.rotationAxis = UnityEngine.Animations.Axis.None;
                pc.translationAxis = UnityEngine.Animations.Axis.Y;
                pc.translationOffsets = new Vector3[] {new Vector3(0f, offset_Y, 0f)};

                var src = new UnityEngine.Animations.ConstraintSource();
                src.sourceTransform = text_rows[current_row].transform;
                src.weight = 1f;
                pc.AddSource(src);
    ...
    public void OnPointerClickSuggestionItem(int n)
    {
        Intellisense_Navigation(nav_enum.Submit, n);
    }
    public void OnPointerEnterSuggestionItem(int n)
    {
        if (suggestion_mouse_enabled) Intellisense_Navigation(nav_enum.Mouse_Scroll, n);
    }
}
...
public class ConcurrentDictionary_Ordered<TKey, TValue>
{
    ...
    public TValue this[int index] { get { return dict[orders[index]]; } set { dict[orders[index]] = value; } }
}
}
        ...
        foreach (KeyCode[] key_arr in Hotkeys[k])
        {
            ...
            foreach (KeyCode kc in key_arr)
            {
                if (kc == KeyCode.LeftAlt || kc == KeyCode.RightAlt)
                    if (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt)) continue;
                    else
                    {
                        ok = false;
                        break;
                ...
                if (kc == KeyCode.LeftShift || kc == KeyCode.RightShift)
                    if (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)) continue;
                    else
                    {
                        ok = false;
                        break;
                ...
                if (kc == KeyCode.LeftControl || kc == KeyCode.RightControl)
                    if (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl)) continue;
                    else
                    {
                        ok = false;
                        break;
                ...
                if (!down && !Input.GetKey(kc))
                {
                    ok = false;
                    break;
                }
                else if (down && !Input.GetKeyDown(kc))
                {
                    ok = false;
                    break;
                }
